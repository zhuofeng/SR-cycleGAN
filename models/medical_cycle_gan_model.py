...
class MedicalCycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_downsample_loss', type=float, default=1.0, help='downsample_loss')
            parser.add_argument('--downsample_loss', type=float, default=1., help='downsample_loss')
            parser.add_argument('--lambda_upsample_loss', type=float, default=1.0, help='upsample_loss')
    ...
    def backward_G(self, epoch):
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_downsample + self.loss_upsample + self.loss_clinical_ssim + self.loss_micro_ssim
